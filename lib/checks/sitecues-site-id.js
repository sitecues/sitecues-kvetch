...
function check(rawContent, $, checkOptions) {
  const
    ...
    RESULT_INVALID_SITE_ID = 'An invalid site id is being used, such as one intended for the extension or testing (%s).'
  ...
  if (badIds.indexOf(match) >= 0) {
    return RESULT_INVALID_SITE_ID.replace('%s', match);
  }
